...
public class ITP_265_Course {
	...
	private ArrayList<Student> gelatoStudents;
	...
	public ITP_265_Course() {
		...
		gelatoStudents = new ArrayList<>(); // make an empty list
	...
	private void makeRandomGroup() {
		if(gelatoStudents.size() + coffeeStudents.size() < 4) {
			...
			ArrayList<Student> students = new ArrayList<>(gelatoStudents);
	...
	private String displaySection(String sectionName) {
		...
		if(sectionName.equalsIgnoreCase("gelato")) {
			...
			for(Student s: this.gelatoStudents) {
				section += s + "\n";
			}
		}
		else {
	...
	private void makeGroup() {
		...
		if(group[0][0] != null) {
			keepGoing = myHelper.inputYesNoAsBoolean("Group already exists, do you want to overwrite previous group? (y/n)");
		}
		else {
	...
	private void makeTAs() {
		...
		allPeople.add(new StudentTA("Malik", 1998, 1, 1, 4.0, "ME", "ITP-265"));
		allPeople.add(new StudentTA("Kamryn", 1998, 1, 1, 4.0, "Neuro", "ITP-265"));
		allPeople.add(new StudentTA("Walter", 1998, 1, 1, 4.0, "Accounting", "ITP-265"));
		allPeople.add(new StudentTA("Anup", 1998, 1, 1, 4.0, "CS", "ITP-265"));
	...
	public void addStudentsViaConsole() {
		...
		while(moreStudents) {
			...
			String section = myHelper.inputWord("Which section, gelato or coffee?", "gelato", "coffee");
			addStudent(s, section);
			moreStudents = myHelper.inputYesNoAsBoolean("Do you want to add another student? (y/n)");
		}
	}

	public void addStudent(Student s, String section) {
		if(section.equalsIgnoreCase("gelato")) {
			gelatoStudents.add(s);
		}
		else {
